...
def read_blog_posts():
    ...
    for filename in glob.glob(f'{parsed_config["public_folder"]}{parsed_config["post_folder"]}*.html'):
        # skip index file
        ...
        with open(filename) as f:
            ...
            # build link
            link = filename.replace(parsed_config['public_folder'], '', 1)
            guid = filename.replace(parsed_config['public_folder'], '', 1)
...
def generate_post_output(posts: List[Dict[str, str]]):
    for post in posts:
        ...
        if headline:
            rss_output.append(f'<title>{html.escape(headline)}</title>')
        if description:
            rss_output.append(
                f'<description>{html.escape(description)}</description>')
        rss_output.append(
            f'<link>{parsed_config["domain"]}/{post.get("link")}</link>')
        rss_output.append(
            f'<guid>{parsed_config["domain"]}/{post.get("guid")}</guid>')
        ...
        if headline:
            element_list.append(
                f'<a href="{post.get("link")}">{html.escape(headline)}</a>')
        if description:
            element_list.append(f'<p>{html.escape(description)}</p>')
        element_list.append('</li>')


def write_files():
    # write rss file
    with open(f'{parsed_config["public_folder"]}feed.rss', "w") as f:
...
rss_output.append(f'<link>{parsed_config["domain"]}</link>')
rss_output.append(f'<description>{parsed_config["description"]}</description>')
rss_output.append(
    f'<atom:link href="{parsed_config["domain"]}/{parsed_config["public_folder"]}feed.rss" rel="self" type="application/rss+xml" />')
