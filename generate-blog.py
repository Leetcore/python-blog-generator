...
if len(new_filename) > 0:
    ...
    with open(template_post_filename, "r") as template:
        with open(f"public/blog/{new_filename}", "w") as new_file:
...
with open("config.toml", "r") as config:
    ...
    for filename in glob.glob("public/blog/*.html", recursive=True):
        with open(filename) as f:
            ...
            # build link
            relative_filename = filename.replace("public", "")
            link = relative_filename
            guid = relative_filename

            rss_output.append(f'<atom:link href="{parsed_config["link"]}/blog/feed.rss" rel="self" type="application/rss+xml" />')
...
# write rss file
with open("public/blog/feed.rss", "w") as f:
